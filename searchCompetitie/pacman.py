...
class GameState:
    ...
    def getLegalPacmanActions( self, agentIndex ):
        return self.getLegalActions( self,agentIndex )
    ...
    def getPacmanPosition( self, agentIndex=0 ):
      #punctul de start pentru agentii Pacman este acelasi
    ...
    def getGhostState( self, agentIndex ):
        if agentIndex == 0 or agentIndex==1 or agentIndex >= self.getNumAgents():
...
class ClassicGameRules:
    ...
    def newGame( self, layout, pacmanAgent, pacmanAgent1, pacmanAgent2, ghostAgents, display, quiet = False, catchExceptions=False):
        agents = [pacmanAgent] + [pacmanAgent1] + [pacmanAgent2]  + ghostAgents[:layout.getNumGhosts()]
        initState = GameState()
    ...
    def process(self, state, game):
        ...
        """
        if state.isWin(): 
            state.data.agentStates[0].isPacman=100
            self.win(state, game)
        if state.isLose(): 
...
def readCommand( argv ):
    ...
    parser.add_option('-b', '--pacman1', dest='pacman1',
                      help=default('the agent TYPE in the pacmanAgents module to use'),
                      metavar='TYPE', default='KeyboardAgent')
    parser.add_option('-s', '--pacman2', dest='pacman2',
                      help=default('the agent TYPE in the pacmanAgents module to use'),
                      metavar='TYPE', default='KeyboardAgent3')                   
    ...
    parser.add_option('-y', '--agentArgs1', dest='agentArgs1',
                      help='Comma separated values sent to agent1. e.g. "opt1=val1,opt2,opt3=val3"')
    ...
    pacman1 = pacmanType1(**agentOpts1) # Instantiate Pacman1 with agentArgs1
...
def replayGame( layout, actions, display ):
    ...
    game = rules.newGame( layout, agents[0], agents[1], agents[2:], display )
    state = game.state
...
def runGames( layout, pacman, pacman1, pacman2, ghosts, display, numGames, record, numTraining = 0, catchExceptions=False, timeout=30 ):
    ...
    for i in range( numGames ):
        ...
        if beQuiet:
            ...
            gameDisplay = display
            rules.quiet = False
        game = rules.newGame( layout, pacman, pacman1, pacman2, ghosts, gameDisplay, beQuiet, catchExceptions)
        game.run()
